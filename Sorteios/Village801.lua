...
function eventTextAreaCallback(id, p, cmd)
    if cmd == "entrada" then
        ui.addTextArea(0, "<p align='center'>Participando", p, 1200, 370, 340, 20, 1, 0x00FF00, 0, false)
        setPlayerNameColorGreen(p)
        entrada(p, #participante + 1)
    elseif cmd == "inicio" then
        ui.addPopup(0, 2, "<p align='center'>Quantidade de sorteados", p, 1620, 430, 200, false)
    ...
    elseif cmd == "discord" then
        ui.addPopup(0, 2, "<p align='center'>❥ Nosso servidor: discord.gg/RDc33tqpGM", p, 620, 430, 340, false)
    elseif cmd == "whatsapp" then
        ui.addPopup(0, 2, "<p align='center'>❥ Canal oficial: chat.whatsapp.com/K0WEHHQAIj2C51hvmGNysH", p, 620, 230, 440, false)
    end
end
...
function eventNewPlayer(p)
    ...
    if not inicio and not fim then
			ui.addTextArea(-15, "<p align='center'><font size='12' color='#ttFFtt'>※ AVISO:<font size='12' color='#9AEBA3'> todos os sabados as 20:30 teremos sorteios de queijos no cafofo da tribo!", nil, 1570, 425, 580, 22, 1, 0x000000, 0.85, false)
			ui.addTextArea(-7, "<p align='center'><a href='event:verGanhadores'>Ver Ganhadores", nil, 1290, 780, 150, 20, 1, 0x00FFFF, 0.5, false)
			ui.addTextArea(0, "<p align='center'><a href='event:entrada'>Participar do sorteio!", nil, 1630, 780, 340, 20, 1, 0xFF0000, 0.8, false)
...
for _, jogador in ipairs(jogadoresBanidos) do
    ...
    tfm.exec.setNameColor(jogador, 0xFF0000) -- Muda a cor para vermelho
end
    ...
    if inicio then
        if fast > 0 then
            sorteioEmAndamento = true  -- Sorteio está em andamento
					...
					ui.addTextArea(1000, "<p align='center'><font color='#E7B200'><b>Processando...", nil, 1600, 760, 400, 20, 1, 0xFECCFF, 0, false)
            ...
            inicio = false
            fim = true
            ui.addTextArea(-12, "<p align='center'><a href='event:novo'>Novo sorteio", adm, 1860, 780, 100, 20, 1, 0xFF00FF, 0.5, false)
            ui.addTextArea(-1, "<p align='center'><a href='event:terminar'>Finalizar SCRIPT", adm, 1990, 780, 150, 20, 1, 0x0000FF, 0.5, false)
            local ganhador = sortearGanhador()
            ...
            if ganhador then
                ...
                eventoGanhador(participante[ganhador])
    				...
    				ui.addTextArea(ganhador, "<p align='center'><font size='30' color='#5CFF00'>" .. participante[ganhador], nil, 1700, 550, 300, 40, 1, 0x5CFF00, 1, false)
                ui.addTextArea(1000, "<p align='center'><font color='#00FF1B'><b>Sorteio encerrado!", nil, 1600, 760, 400, 20, 1, 0x6E27FD, 0, false)
					tfm.exec.movePlayer(participante[ganhador], 1720, 540)
...
function mostrarGanhador(ganhador)
    ...
    ui.addTextArea(ganhador, "<p align='center'><font size='30' color='" ..corTexto.. "'>" .. participante[ganhador], nil, 1700, 550, 400, 80, 1, 1, 0, false)
end
...
function eventTextAreaCallback(id, p, cmd)
    if cmd == "entrada" then
        ui.addTextArea(0, "<p align='center'>Participando", p, 1630, 780, 340, 20, 1, 0x00FF00, 0.8, false)
        entrada(p, #participante + 1)
        setPlayerNameColorGreen(p)
    elseif cmd == "inicio" then
        ui.addPopup(0, 2, "<p align='center'>Quantidade de sorteados", p, 1620, 630, 200, false)
    elseif cmd == "terminar" then
        terminar()
    elseif cmd == "reviver" then
        reviver(p)
...
function verLista(p)
	...
	ui.addTextArea(-5, listaParticipantes .. "<br><br><p align='center'><a href='event:fechar'>Fechar</a>", p, 1900, 430, 400, 250, 1, 1, 0.6, false)
end
...
function verGanhadores(p)
    ...
    ui.addTextArea(-6, listaGanhadores .. "<br><br><p align='center'><a href='event:fecharGanhadores'>Fechar</a>", p, 1300, 430, 400, 250, 1, 1, 0.6, false)
end
...
function showMessage(message,name)
		...
		if tfm.get.room.isTribeHouse == false then
			...
			ui.addPopup(0,0,temp_text,name,1650,675,400,true)
		end
end

-- STAFFS COR
tfm.exec.setNameColor("Bleseaze#8245", 0x9D00FF)
tfm.exec.setNameColor("Trevas#4503", 0x9D00FF)

tfm.exec.movePlayer("Bleseaze#8245", 1650, 580)
tfm.exec.movePlayer("Trevas#4503", 1660, 580)
...
ui.addTextArea(-16, "<p align='center'><font size='12' color='#ttFFtt'>※ AVISO:<font size='12' color='#8AEBG3'> Participe em nossas redes sociais clicando no link abaixo!", nil, 1570, 425, 580, 22, 1, 0x000000, 0.85, false)
ui.addTextArea(-7, "<p align='center'><a href='event:verGanhadores'>Ver Ganhadores", nil, 1290, 780, 150, 20, 1, 0x00FFFF, 0.5, false)
ui.addTextArea(0, "<p align='center'><a href='event:entrada'>Participar do sorteio!", nil, 1630, 780, 340, 20, 1, 0xFF0000, 0.8, false)
ui.addTextArea(-1, "<p align='center'><a href='event:inicio'>Iniciar", adm, 1990, 780, 150, 20, 1, 0x0000FF, 0.5, false)
ui.addTextArea(-2, "<p align='center'><a href='event:lista'>Lista de participantes", nil, 1460, 780, 150, 20, 1, 0xFF00FF, 0.5, false)
